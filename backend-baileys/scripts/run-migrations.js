...
async function main() {
  const defaultFiles = [
    ...
    path.join(__dirname, '..', 'database', 'migration_v28.sql'),
    path.join(__dirname, '..', 'database', 'migration_billing.sql'),
  ];
  ...
  try {
    ...
    const [tables] = await conn.query(`
      ...
      WHERE TABLE_SCHEMA = ? AND TABLE_NAME IN (
        ...
        'billing_plans','billing_orders','account_licenses','payment_gateway_settings','payment_transactions'
      )
      ORDER BY TABLE_NAME
    `, [DB.database]);
