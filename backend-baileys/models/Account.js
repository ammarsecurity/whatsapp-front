...
class Account {
  ...
  static async updateNote(accountId, userId, note) {
    const value = note ? String(note).trim().slice(0, 160) : null;
    try {
      await pool.execute(
        'UPDATE accounts SET note = ? WHERE account_id = ? AND user_id = ?',
        [value, accountId, userId],
      );
    } catch (err) {
      if (err.code !== 'ER_BAD_FIELD_ERROR' && !String(err.message || '').includes('note')) {
        throw err;
      }
      await pool.query(
